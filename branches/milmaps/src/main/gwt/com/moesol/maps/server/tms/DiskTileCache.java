...
public class DiskTileCache implements ITileImageCache {
	...
	private class NoOpTileCacheImpl implements ITileImageCache {
		...
		@Override
		public void addImage(String layerId, int level, int x, int y,
				BufferedImage image) {
		}
	}
	...
	private class DiskTileCacheImpl implements ITileImageCache {
			...
			if (imageFile.exists()) {
				try {
					...
					LOGGER.log(Level.WARNING,
							"Failed reading image file from disk", e);
				}
			}
			return null;
		}
		...
		private File getImageFile(String layerId, int level, int x, int y) {
			String filepath = m_baseDirectory + "/" + layerId + "/" + level + "/"
					+ x + "/" + y + ".png";
	...
	private static final Logger LOGGER = Logger.getLogger(DiskTileCache.class
			.getName());
	...
	public DiskTileCache(String baseDirectory, String imageFormat) {
		...
		if (hasAccessToBaseDirectory(baseDirectory)) {
			...
			LOGGER.log(Level.INFO, "Caching tiles at '"
					+ baseDirectory + "'");
		} else {
			m_impl = new NoOpTileCacheImpl();
			LOGGER.log(Level.SEVERE, "Unable to read and write directory '"
					+ baseDirectory + "'");
		}
	}
