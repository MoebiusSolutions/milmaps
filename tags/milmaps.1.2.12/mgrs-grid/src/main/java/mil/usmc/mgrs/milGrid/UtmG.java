...
public class UtmG {
    ...
    //protected int[] m_iStartBotEw = new int[1];
    ...
    {
      int gridSpace = 5;
      if ( m_dWidthDeg >  90  )// 200 degrees * 60 Nms * 1853 meters.
        gridSpace = 5;
      else if ( m_dWidthDeg > 22.5  )// 100 degrees * 60 Nms * 1853 meters.
        gridSpace = 3;
      else if ( m_dWidthDeg > 5  )// 50 degrees * 60 Nms * 1853 meters.
        gridSpace = 2;
      else 
        gridSpace = 1; 
      return gridSpace;
    ...
    {
    	// Loop through each zone and draw them.
        ...
        )
        {
        	...
        	while ( m_iBotEw[0] < m_iEndBotEw[0] ) {
        	    ...
        	    if ( Math.abs(iTopNextEw-iBotNextEw) < iInc/2 ){
        	        ...
        	        boolean bDraw = true;
        	        if ( maxX < 1 || minX > iWidth )
        	        	bDraw = false;
        	        if( maxY < 1 || minY > iHeight )
        	        	bDraw = false;
	...
	)
	{
		...
		int vWidth = box.getWidth()+1;
      	...
      	switch ( m_gridSize ){
      		case e_100K :
      			// Draw the MGR label.
      			// Draw the label for the cell top and right to this point
      			...
      			if ( csMgr == null || csMgr.length() < 1 )
      				return;
      			csStr = csMgr.substring(4, 6);//(4,2);
      			cx  = 2*fm.charWidth(csStr.charAt(0)); 
      			if ( Math.abs(pPt[nTL].m_x-pPt[nTR].m_x) < 1.5*cx || pt.m_y < -2)
      				return;
      			g.drawString(csStr, pt.m_x-cx, pt.m_y + 15);
      		break;
      		case e_10K :
      		case e_1K :
	    	// Draw the 1K and 10K lines.
	    	tlX = pPt[nTL].m_x;
	      	tlY = pPt[nTL].m_y;
	      	trX = pPt[nTR].m_x;
	      	trY = pPt[nTR].m_y;
	      	...
	      	// Check to see if we need to place the vertical line label
	      	if ( 0 < brX  && brX < vWidth ){
	      		// Vertical line label
