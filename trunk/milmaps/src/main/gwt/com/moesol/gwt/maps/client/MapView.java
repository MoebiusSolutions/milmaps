...
public class MapView extends Composite implements SourcesChangeEvents {
	...
	private final IProjection m_projection = new CylEquiDistProj();
	private final IProjection m_tempProj = new CylEquiDistProj();
	...
	public MapView(double defLng, double defLat) {
		ProjectionValues.readCookies(m_projection);
		setCenter(recoverCenter(defLng, defLat));
		initSize();
		updateView();
	...
	public void compTileDegDimensions() {
		double degW = m_projection.pixToLngDegMag(m_viewPort.getTilePixWidth());
		WorldCoords wc = m_projection.geodeticToWorld(m_projection
				.getVpGeoCenter());
		int centY = wc.getY();
		double degH = m_projection.pixToLatDegMag(centY,
				m_viewPort.getTilePixHeight());
		m_viewPort.setTileDegWidth(degW);
		m_viewPort.setTileDegHeight(degH);
	}
	...
	public void setLayerBestSuitedForScale(ZoomFlag zoomFlag) {
		...
		if (bestLayerSoFar != null) {
			...
			// double scale = syncProjScale(zoomFlag, bestScaleSoFar);
			// m_projection.setScale(scale);
		}
	}
	...
	private boolean hasDynamicLayers() {
		for (TiledImageLayer layer : m_tiledImageLayers) {
			if (layer.getLayerSet().isDynamic()) {
				return true;
			}
		}
		return false;
	...
	public boolean moveMapByPixels(int dx, int dy) {
		if (m_bSuspendMapAction == false) {
			...
			setCenter(m_projection.getVpGeoCenter());
			updateView();
		}
	...
	public boolean zoomOnPixel(int x, int y, double scaleFactor) {
		...
		if (m_bSuspendMapAction == false) {
			m_vc.setX(x).setY(y);
			GeodeticCoords gc = m_projection.viewToGeodetic(m_vc);
			if (bZoomIn) {
			...
			m_projection.tagPositionToPixel(gc, m_vc);
			setCenter(m_projection.getVpGeoCenter());
	...
	public void flyTo(BoundingBox box) {
		double cLat = box.getCenterLat();
		double cLng = box.getCenterLng();
		
		double pixWidth = m_projection.lngDegToPixMag(box.getLonSpan());
		double pixHeight = m_projection.latDegToPixMag(cLat, box.getLatSpan());
		double zoomX = 1.0;
		...
		m_flyToEngine.flyTo(cLat, cLng, zoomX);
	}
